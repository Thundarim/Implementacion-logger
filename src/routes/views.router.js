...
router.delete("/products/:pid", isLoggedIn, isAdmin, async (req, res) => {
    try {
        ...
        req.logger.error("Error al eliminar producto por ID:", error);
        res.status(500).json({ error: "Error interno del servidor" });
    }
});
...
router.get("/cart", isLoggedIn, (req, res) => {
    ...
    if (req.session.user.role === 'admin') {
        cartManager.getCarts()
            ...
            .catch(error => {
                req.logger.error("Error al hacer fetch a los carritos:", error);
                res.status(500).json({ error: "Error interno del servidor" });
            });
    } else {
        ...
        cartManager.getCartById(cartId)
            ...
            .catch(error => {
                req.logger.error("Error fetching user's cart:", error);
                res.status(500).json({ error: "Error interno del servidor" });
            });
    }
});
...
router.get('/carts/:cid', isLoggedIn, async (req, res) => {
    try {
        ...
        req.logger.error("Error al obtener productos en el carrito:", error);
        res.status(500).json({ error: "Error interno del servidor" });
    }
});

router.get('/mockview', (req, res) => {
    res.render('mockview');
});
