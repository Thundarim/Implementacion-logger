...
router.post("/products", async (req, res) => {
    try {
        ...
        req.logger.log("Producto agregado correctamente");
...
router.put("/products/:pid", async (req, res) => {
    try {
        ...
        req.logger.error("Error al actualizar producto por ID:", error);
        res.status(500).json({ error: "Error interno del servidor" });
    }
});
...
router.delete("/products/:idOrCode", async (req, res) => {
    try {
        ...
        req.logger.error("Error al eliminar producto por ID o código:", error);
